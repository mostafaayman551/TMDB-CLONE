...
const RecommendationMedia = ({ mediaType, id }) => {
  ...
  return (
    <>
      {data?.length > 0 && (
        <>
          <h2 className="text-2xl text-white font-bold mb-2">Recommended</h2>
            ...
            <div className="flex gap-4">
              {data?.slice(0, 20).map((item) => (
                <Link
                  ...
                  key={item?.id}
                  className="rounded-lg shadow-md bg-white/80 text-center w-[240px] flex-shrink-0 overflow-hidden relative will-change-transform isolate hover:scale-105 hover:translate-x-[30px] hover:z-50 hover:bg-transparent transition-all duration-300 group"
                >
                  <LazyLoadImage
                    src={
                      item?.poster_path
                  ...
                  <div className="absolute bottom-[-100%] left-0 w-full bg-white/80 text-black font-black  text-wrap text-sm leading-relaxed text-center p-1 group-hover:bottom-0 transition-all duration-300">
                    <p className="first-letter:text-purple-500 first-letter:font-bold first-letter:text-lg line-clamp-[10]">Overview: {item?.overview}</p>
                  </div>
                </Link>
              ))}
            </div>
